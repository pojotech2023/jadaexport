<?php include '../header.php'; ?>
<!-- ::::::  Start  Breadcrumb Section  ::::::  -->
<div class="page-breadcrumb">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <ul class="page-breadcrumb__menu">
                    <li class="page-breadcrumb__nav"><a href="../index.php">Home</a></li>
                    <li class="page-breadcrumb__nav active"><a href="../product.php">Products</a></li>
                    <li class="page-breadcrumb__nav active">Dhall</li>
                </ul>
            </div>
        </div>
    </div>
</div> <!-- ::::::  End  Breadcrumb Section  ::::::  -->

<!-- :::::: Start Main Container Wrapper :::::: -->
<main id="main-container" class="main-container">
    <div class="container-fluid">
        <div class="row">
            <div class="product__box product__default--single text-center col-md-4">
                <div class="product__img-box  pos-relative">
                    <a href="toordhall.php" class="product__img--link">
                        <img class="product__img img-fluid" src="../assets/img/product/dhall/toordhal.png" alt="">
                    </a>

                    <ul class="product__action--link pos-absolute">
                        <li><a href="toordhall.php"><i class="icon-eye"></i></a></li>
                    </ul>
                </div>

                <div class="product__content m-t-20">
                    <a href="toordhall.php" class="product__link">TOOR DHALL</a>
                    <div class="product__price m-t-5">
                        <a href="toordhall.php"><span class="product__price">Enquiry </span></a>
                    </div>
                </div>
            </div>

            <div class="product__box product__default--single text-center col-md-4">
                <div class="product__img-box  pos-relative">
                    <a href="uraddhall.php" class="product__img--link">
                        <img class="product__img img-fluid" src="../assets/img/product/dhall/uraddhall.png" alt="">
                    </a>

                    <ul class="product__action--link pos-absolute">
                        <li><a href="uraddhall.php"><i class="icon-eye"></i></a></li>
                    </ul>
                </div>

                <div class="product__content m-t-20">
                    <a href="uraddhall.php" class="product__link">URAD DHALL</a>
                    <div class="product__price m-t-5">
                        <a href="uraddhall.php"><span class="product__price">Enquiry </span></a>
                    </div>
                </div>
            </div>
            <div class="product__box product__default--single text-center col-md-4">
                <div class="product__img-box  pos-relative">
                    <a href="green_moong.php" class="product__img--link">
                        <img class="product__img img-fluid" src="../assets/img/product/dhall/Moong-Dal.webp" alt="" style="height: 390px;">
                        <img class="product__logo" src="../assets/img/logo/logo-1.jpeg" alt="Company Logo">
                    </a>

                    <ul class="product__action--link pos-absolute">
                        <li><a href="green_moong.php"><i class="icon-eye"></i></a></li>
                    </ul>
                </div>

                <div class="product__content m-t-20">
                    <a href="green_moong.php" class="product__link">GREEN MOONG DHALL</a>
                    <div class="product__price m-t-5">
                        <a href="green_moong.php"><span class="product__price">Enquiry </span></a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</main> <!-- :::::: End MainContainer Wrapper :::::: -->

<?php include '../footer.php'; ?>